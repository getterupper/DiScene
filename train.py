...
def main():
    ...
    run_name += '_' + datetime.now().strftime("%Y-%m-%d/%H-%M")
    if cfgs.dataset_type is not None:
        work_dir = os.path.join('outputs', cfgs.dataset_type, cfgs.model.type, run_name)
    else:
        work_dir = os.path.join('outputs', cfgs.train_dataset_type, cfgs.model.type, run_name)
    ...
    logging.info('Setting random seed: 42')
    ...
    if world_size > 1:
        model = MMDistributedDataParallel(model, [local_rank], broadcast_buffers=False, find_unused_parameters=False)
    else:
    ...
    if cfgs.model.type == 'DiScene_Distill' or cfgs.model.type == 'DiScene_Distill_Vanilla':
        model.module.load_teacher()
