...
class OccScanNetDataset_Distill(Dataset):
    def __init__(
        self,
        data_path, 
        num_frames=1,
        ...
        resize_lim=[1.0, 1.0],
        num_pts=21600,
        teacher_pretrained_depth_model='Metric3Dv2-Giant',
        student_pretrained_depth_model='DepthAnythingV2',
        data_tg='base'
        ):
        super(OccScanNetDataset_Distill, self).__init__()
        ...
        self.teacher_pretrained_depth_model = teacher_pretrained_depth_model
        self.student_pretrained_depth_model = student_pretrained_depth_model
    ...
    def get_data_info(self, index):
        ...
        if self.teacher_pretrained_depth_model == 'DepthAnythingV2':
            transform = Compose([
                Resize(
                    width=480,
                    height=480,
        ...
        elif self.teacher_pretrained_depth_model == 'DepthAnythingV1':
            ...
            meta['img_depthbranch'] = img_depthbranch
            meta['depth_gt_valid'] = depth_gt
        elif self.teacher_pretrained_depth_model == 'Metric3Dv2-Small':
            depth_gt_np = cv2.resize(depth_gt_np, (640, 480), interpolation=cv2.INTER_NEAREST)
            depth_gt = torch.from_numpy(depth_gt_np).unsqueeze(0)
            meta['depth_gt'] = depth_gt
            depth_valid_mask = (torch.isnan(depth_gt) == 0)
        ...
        elif self.teacher_pretrained_depth_model == 'Metric3Dv2-Giant':
            depth_gt_np = cv2.resize(depth_gt_np, (640, 480), interpolation=cv2.INTER_NEAREST)
            depth_gt = torch.from_numpy(depth_gt_np).unsqueeze(0)
            meta['depth_gt'] = depth_gt
            depth_valid_mask = (torch.isnan(depth_gt) == 0)
            ...
            meta['normalize_scale'] = normalize_scale
            meta['label_scale_factor'] = label_scale_factor
        elif self.teacher_pretrained_depth_model == 'GT' or self.teacher_pretrained_depth_model == 'None':
            transform = Compose([
                Resize(
                    width=480,
                    height=480,
        ...
        if self.student_pretrained_depth_model == 'DepthAnythingV2':
            transform = Compose([
                Resize(
                    width=480,
                    height=480,
            ...
            img_depthbranch = torch.from_numpy(sample['image']).unsqueeze(0)
            depth_gt = torch.from_numpy(sample['depth']).unsqueeze(0)
            meta['student_depth_gt'] = depth_gt
            depth_valid_mask = (torch.isnan(depth_gt) == 0)
            depth_gt[depth_valid_mask == 0] = 0
            meta['student_img_depthbranch'] = img_depthbranch
            meta['student_depth_gt_valid'] = depth_gt
        elif self.student_pretrained_depth_model == 'DepthAnythingV1':
            img_depthbranch = Image.open(rgb_path).convert("RGB")
            img_depthbranch = img_depthbranch.resize((640, 480))
            depth_gt_np = cv2.resize(depth_gt_np, (640, 480), interpolation=cv2.INTER_NEAREST)
            depth_gt = torch.from_numpy(depth_gt_np).unsqueeze(0)
            ...
            meta['student_img_depthbranch'] = img_depthbranch
            meta['student_depth_gt_valid'] = depth_gt
        elif self.student_pretrained_depth_model == 'Metric3Dv2-Small':
            depth_gt_np = cv2.resize(depth_gt_np, (640, 480), interpolation=cv2.INTER_NEAREST)
            depth_gt = torch.from_numpy(depth_gt_np).unsqueeze(0)
            meta['student_depth_gt'] = depth_gt
            depth_valid_mask = (torch.isnan(depth_gt) == 0)
            ...
            rgb_input, _, pad, label_scale_factor = transform_test_data_scalecano(rgb_origin, intrinsic, data_basic)
            rgb_input = rgb_input[None, ...]
            meta['student_img_depthbranch'] = rgb_input
            meta['student_pad'] = pad
            meta['student_normalize_scale'] = normalize_scale
            meta['student_label_scale_factor'] = label_scale_factor
        elif self.student_pretrained_depth_model == 'Metric3Dv2-Giant':
            depth_gt_np = cv2.resize(depth_gt_np, (640, 480), interpolation=cv2.INTER_NEAREST)
            depth_gt = torch.from_numpy(depth_gt_np).unsqueeze(0)
            meta['student_depth_gt'] = depth_gt
            depth_valid_mask = (torch.isnan(depth_gt) == 0)
            ...
            meta['student_normalize_scale'] = normalize_scale
            meta['student_label_scale_factor'] = label_scale_factor
        elif self.student_pretrained_depth_model == 'GT' or self.student_pretrained_depth_model == 'None':
            transform = Compose([
                Resize(
                    width=480,
                    height=480,
