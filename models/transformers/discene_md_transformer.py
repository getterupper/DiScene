...
from .discene_sampling import sampling_4d_discene
...
class DiSceneTransformer_MD(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=1,
        ...
        self.decoder = DiSceneTransformerDecoder_MD(
    ...
    def forward(self, query_points, query_feat, mlvl_feats, depth_preds, img_metas):
        cls_scores, refine_pts, query_feats = self.decoder(
            query_points, query_feat, mlvl_feats, depth_preds, img_metas)

        cls_scores = [torch.nan_to_num(score) for score in cls_scores]
        refine_pts = [torch.nan_to_num(pts) for pts in refine_pts]
        query_feats = [torch.nan_to_num(feat) for feat in query_feats]

        return cls_scores, refine_pts, query_feats


class DiSceneTransformerDecoder_MD(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=1,
        ...
        for i in range(num_layers):
            self.decoder_layers.append(
                DiSceneTransformerDecoderLayer_MD(
    ...
    def forward(self, query_points, query_feat, mlvl_feats, depth_preds, img_metas):
        ...
        # TODO: indoor occupancy (is that correct?)
        world2cam = np.asarray([m['world2cam'] for m in img_metas]).astype(np.float32)
        world2cam = query_feat.new_tensor(world2cam) # [B, 4, 4]
        cam_k = np.asarray([m['cam_k'] for m in img_metas]).astype(np.float32)
        cam_k = query_feat.new_tensor(cam_k) # [B, 3, 3]
        ...
        for i, decoder_layer in enumerate(self.decoder_layers):
            ...
            query_feat, cls_score, query_points = decoder_layer(
                query_points, query_feat, mlvl_feats, depth_preds, world2cam, cam_k, img_metas)
...
class DiSceneTransformerDecoderLayer_MD(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=1,
        ...
        self.self_attn = DiSceneSelfAttention_MD(
            embed_dims, num_heads=8, dropout=0.1, pc_range=pc_range)
        self.sampling = DiSceneSampling_MD(embed_dims, num_frames=num_frames, num_views=num_views,
                                     num_groups=num_groups, num_points=num_points, 
                                     num_levels=num_levels, pc_range=pc_range)
        self.mixing = DiSceneAdaptiveMixing_MD(in_dim=embed_dims, in_points=num_points * num_frames,
        ...
        self.depth_aware_layer = DepthAwareLayer(2 * self.last_refines, self.embed_dims)
    ...
    def forward(self, query_points, query_feat, mlvl_feats, depth_preds, world2cam, cam_k, img_metas):
        """
        ...
        depth_feats = []
        assert B == len(depth_preds)
        for i in range(B):
            cam_E_i = world2cam[i, ...]
            cam_k_i = cam_k[i, ...]
            depth_pred = depth_preds[i]

            vox_origin = np.asarray(img_metas[i]['vox_origin']).astype(np.float32)
            vox_origin = query_feat.new_tensor(vox_origin)

            sample_points = query_points[i, ...].clone().detach()
            N, G, C = sample_points.shape
            sample_points = sample_points.reshape(N*G, C)
            sample_points = decode_points(sample_points, self.pc_range)
            sample_points = sample_points + vox_origin

            ones = torch.ones_like(sample_points[..., :1])
            sample_points = torch.cat([sample_points, ones], dim=-1)
            sample_points = sample_points.unsqueeze(-1) # [N, 4, 1]
            cam_E_i = cam_E_i.expand(N*G, 4, 4)
            cam_pts = torch.matmul(cam_E_i, sample_points).squeeze(-1)
            cam_pts = cam_pts[..., :3]
            pix_depth = cam_pts[..., 2:3] # [N, 1]

            sample_points_cam = torch.empty((*cam_pts.shape[:-1], 2), dtype=sample_points.dtype)
            fx, fy = cam_k_i[0, 0], cam_k_i[1, 1]
            cx, cy = cam_k_i[0, 2], cam_k_i[1, 2]
            sample_points_cam[:, 0] = cam_pts[:, 0] * fx / cam_pts[:, 2] + cx
            sample_points_cam[:, 1] = cam_pts[:, 1] * fy / cam_pts[:, 2] + cy

            anchor_pix_x = sample_points_cam[:, 0:1]
            anchor_pix_y = sample_points_cam[:, 1:2]
            ...
            anchor_depth = depth_pred[anchor_pix_y, anchor_pix_x]

            anchor_depth_input = torch.concat((anchor_depth, pix_depth), dim=-1)
            anchor_depth_input = anchor_depth_input.reshape(N, G*2)
            anchor_depth_feature = self.depth_aware_layer(anchor_depth_input)

            depth_feats.append(anchor_depth_feature)
        depth_feat = torch.stack(depth_feats, dim=0)
        query_feat = query_feat + depth_feat

        query_pos = self.position_encoder(query_points.flatten(2, 3))
        query_feat = query_feat + query_pos

        sampled_feat = self.sampling(
            query_points, query_feat, mlvl_feats, world2cam, cam_k, img_metas)
        ...
        if DUMP.enabled and DUMP.stage_count == 5:
            query_points_cam = torch.cat([anchor_pix_x, anchor_pix_y, pix_depth.cpu()], dim=-1).unsqueeze(0)
            cls_score_sig = cls_score.sigmoid()
            torch.save(query_points_cam.cpu(), '{}/query_points_cam_stage{}.pth'.format(DUMP.out_dir, DUMP.stage_count)) # [B, Q, 3]
            torch.save(cls_score_sig.cpu(), '{}/cls_score_stage{}.pth'.format(DUMP.out_dir, DUMP.stage_count)) # [B, Q, P, num_classes]

        return query_feat, cls_score, refine_pt


class DiSceneSelfAttention_MD(BaseModule):
...
class DiSceneSampling_MD(BaseModule):
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, world2cam, cam_k, img_metas):
        '''
        query_points: [B, Q, 6]
        query_feat: [B, Q, C]
        '''
        ...
        for i in range(B):
            ...
            vox_origin_i = img_meta['vox_origin']
            vox_origins.append(vox_origin_i)
        ...
        # sampling
        sampled_feats = sampling_4d_discene(
            sampling_points,
            mlvl_feats,
            scale_weights,
            world2cam,
    ...
    def forward(self, query_points, query_feat, mlvl_feats, world2cam, cam_k, img_metas):
        if self.training and query_feat.requires_grad:
            return cp(self.inner_forward, query_points, query_feat, mlvl_feats,
                      world2cam, cam_k, img_metas, use_reentrant=False)
        else:
            return self.inner_forward(query_points, query_feat, mlvl_feats,
                                      world2cam, cam_k, img_metas)


class DiSceneAdaptiveMixing_MD(nn.Module):
    """Adaptive Mixing"""
