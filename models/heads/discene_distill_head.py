...
class DiSceneHead_Distill(BaseModule):
    def __init__(self,
                 num_classes,
                 in_channels,
                 num_query,
    ...
    def forward(self, mlvl_feats, img_metas, is_train=False):
        ...
        for i in range(B):
            if self.pretrained_depth_model == 'DepthAnythingV1':
                image_ = img_metas[i]['student_img_depthbranch']
                ...
                image_ = img_metas[i]['student_img_depthbranch'].to(mlvl_feats[0].device)
                ...
                pad = img_metas[i]['student_pad']
                normalize_scale = img_metas[i]['student_normalize_scale']
                label_scale_factor = img_metas[i]['student_label_scale_factor']
                ...
                depth_pred = img_metas[i]['student_depth_gt'].to(mlvl_feats[0].device).squeeze(0)  # using GT
            else:
                raise NotImplementedError
