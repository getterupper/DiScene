dataset_type = 'OccScanNetDataset_Distill'
...
teacher_weight = 'checkpoints/teacher.pth'  # NOTE:  modify it to /path/to/your/teacher/model
teacher_img_backbone = dict(
    type='InternImage',
    core_op='DCNv3',
    channels=192,
    depths=[5, 5, 24, 5],
    ...
    with_cp=False,
    out_indices=(0, 1, 2, 3),
    init_cfg=dict(type='Pretrained', checkpoint=teacher_weight)
)
teacher_img_neck = dict(
    type='FPN',
...
teacher_model = dict(
    ...
    data_aug=None,
    stop_prev_grad=0,
    img_backbone=teacher_img_backbone,
    img_neck=teacher_img_neck,
    pts_bbox_head=dict(
        ...
        pretrained_depth_model=teacher_pretrained_depth_model),
    train_cfg=dict(
        pts=dict(
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            loss_weight=2.0),
        loss_pts=dict(type='SmoothL1Loss', beta=0.04, loss_weight=0.5),
        pretrained_depth_model=student_pretrained_depth_model),
    train_cfg=dict(
        pts=dict(
            cls_freq=[4393702080, 700497, 44393812, 39153315, 3367920, 21860824, 10667655, 13832443, 23358785, 263684, 30415451, 9338079],
            cls_freq_offset=0.05,
...
data = dict(
    ...
    train=dict(
        ...
        teacher_pretrained_depth_model=teacher_pretrained_depth_model,
        student_pretrained_depth_model=student_pretrained_depth_model,
        data_tg='base',
        ),
    val=dict(
        type=dataset_type,
